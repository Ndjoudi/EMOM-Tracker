...
function App(){
  ...
  const startWorkout=(routine,context={})=>{
    const wo={id:window.uid(),routineId:routine.id,routineName:routine.name,startedAt:Date.now(),
      ...
      exercises:routine.exerciseRefs.map(ref=>{
        const libEx=exLib.find(e=>e.id===ref.exId);if(!libEx)return null;
        const base={exId:libEx.id,name:libEx.name,rm:libEx.rm,nbSets:ref.nbSets,emomTime:ref.emomTime,restTime:ref.restTime,
          sets:Array.from({length:ref.nbSets},(_,i)=>{const p=window.getLastPerf(history,libEx.name,i);return{kg:p?p.kg:'',reps:p?p.reps:'',done:false};})};
  ...
  const finishWorkout=wo=>{
    const entry={id:wo.id,routineName:wo.routineName,date:wo.startedAt,duration:Date.now()-wo.startedAt,sleep:wo.sleep,nutrition:wo.nutrition,
      exercises:wo.exercises.map(ex=>{const e={name:ex.name,exId:ex.exId,rm:ex.rm,sets:ex.sets};
        if(ex.pyramid){e.pyramid=true;e.pyGrid=ex.pyGrid;e.pyRows=ex.pyRows;e.pyCols=ex.pyCols;e.pyX=ex.pyX;e.pyY=ex.pyY;}
        return e;})};
  ...
  if(screen==='transfer') return<TransferScreen exLib={exLib} routines={routines} history={history} circuits={circuits} circuitHistory={circuitHistory} onImport={d=>{if(d.exLib)setExLib(d.exLib);if(d.routines)setRoutines(d.routines);if(d.history)setHistory(d.history);if(d.circuits)setCircuits(d.circuits);if(d.circuitHistory)setCircuitHistory(d.circuitHistory);}} onBack={()=>setScreen('home')}/>;
