  ...
  return (
    <div style={S.app}>
      ...
      {showAdd && <Modal onClose={() => setShowAdd(false)}>
        ...
        <div style={{ height: 12 }}/>
        <button onClick={() => setShowAdd(p => ({ ...p, py: !p.py }))} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', cursor: 'pointer', padding: 0, width: '100%' }}>
          <div style={{ width: 36, height: 20, borderRadius: 10, background: showAdd.py ? S.orange : '#333', transition: 'background .2s', position: 'relative', flexShrink: 0 }}>
            <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#fff', position: 'absolute', top: 2, left: showAdd.py ? 18 : 2, transition: 'left .2s' }}/>
          </div>
          <span style={{ fontSize: 14, color: showAdd.py ? S.orange : '#aaa' }}>Mode pyramide par défaut</span>
        </button>
        {showAdd.py && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginTop: 10 }}>
            <div><label style={S.lbl}>Lignes</label><input style={S.inpS} type="number" value={showAdd.pyRows ?? 3} onChange={(e) => setShowAdd(p => ({ ...p, pyRows: Math.max(1, parseInt(e.target.value) || 1) }))}/></div>
            <div><label style={S.lbl}>Colonnes</label><input style={S.inpS} type="number" value={showAdd.pyCols ?? 5} onChange={(e) => setShowAdd(p => ({ ...p, pyCols: Math.max(1, parseInt(e.target.value) || 1) }))}/></div>
            <div><label style={S.lbl}>X (sec)</label><input style={S.inpS} type="number" value={showAdd.pyX ?? 60} onChange={(e) => setShowAdd(p => ({ ...p, pyX: Math.max(5, parseInt(e.target.value) || 60) }))}/></div>
            <div><label style={S.lbl}>Y (sec)</label><input style={S.inpS} type="number" value={showAdd.pyY ?? 120} onChange={(e) => setShowAdd(p => ({ ...p, pyY: Math.max(0, parseInt(e.target.value) || 120) }))}/></div>
          </div>
        )}
        <div style={{ height: 16 }}/>
        <button style={S.btn} onClick={() => { if (!name.trim()) return; setExLib((p) => [...p, { id: window.uid(), name: name.trim(), rm, bodyweight: !!showAdd.bw, pyramid: !!showAdd.py, pyRows: showAdd.pyRows ?? 3, pyCols: showAdd.pyCols ?? 5, pyX: showAdd.pyX ?? 60, pyY: showAdd.pyY ?? 120 }]); setShowAdd(false); }}>Ajouter</button>
      </Modal>}
      ...
      {editEx && <Modal onClose={() => setEditEx(null)}>
        ...
        <button onClick={() => setEditEx(p => ({ ...p, pyramid: !p.pyramid }))} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', cursor: 'pointer', padding: 0, width: '100%', marginBottom: 12 }}>
          <div style={{ width: 36, height: 20, borderRadius: 10, background: editEx.pyramid ? S.orange : '#333', transition: 'background .2s', position: 'relative', flexShrink: 0 }}>
            <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#fff', position: 'absolute', top: 2, left: editEx.pyramid ? 18 : 2, transition: 'left .2s' }}/>
          </div>
          <span style={{ fontSize: 14, color: editEx.pyramid ? S.orange : '#aaa' }}>Mode pyramide par défaut</span>
        </button>
        {editEx.pyramid && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginBottom: 16 }}>
            <div><label style={S.lbl}>Lignes</label><input style={S.inpS} type="number" value={editEx.pyRows ?? 3} onChange={(e) => setEditEx(p => ({ ...p, pyRows: Math.max(1, parseInt(e.target.value) || 1) }))}/></div>
            <div><label style={S.lbl}>Colonnes</label><input style={S.inpS} type="number" value={editEx.pyCols ?? 5} onChange={(e) => setEditEx(p => ({ ...p, pyCols: Math.max(1, parseInt(e.target.value) || 1) }))}/></div>
            <div><label style={S.lbl}>X (sec)</label><input style={S.inpS} type="number" value={editEx.pyX ?? 60} onChange={(e) => setEditEx(p => ({ ...p, pyX: Math.max(5, parseInt(e.target.value) || 60) }))}/></div>
            <div><label style={S.lbl}>Y (sec)</label><input style={S.inpS} type="number" value={editEx.pyY ?? 120} onChange={(e) => setEditEx(p => ({ ...p, pyY: Math.max(0, parseInt(e.target.value) || 120) }))}/></div>
          </div>
        )}
