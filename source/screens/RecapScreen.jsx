...
window.RecapScreen = function RecapScreen({ data, history, profile, onHome, onSaveProfile }) {
  ...
  const exStats = entry.exercises.map(ex => {
    ...
    if (ex.pyramid && ex.pyGrid) {
      ...
      const prevGrid = history.filter(h => h.id !== entry.id)
        .flatMap(h => h.exercises).find(e => e.name === ex.name && e.pyGrid)?.pyGrid;
      ...
      return { ...ex, doneSets: filled.map(v => ({ kg:'', reps:v, done:true })), allCompleted,
        isBW:true, maxKg:0, vol:totalReps, totalReps,
        volDelta: prevReps && prevReps > 0 ? Math.round((totalReps - prevReps)/prevReps*100) : null,
        isPR:false, isFirstSession: prevReps === null && totalReps > 0,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {ex.pyramid
                  ? <span style={{ background: '#F59E0B22', color: '#F59E0B', fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 5 }}>PYRAMIDE</span>
              ...
              <div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>
                {ex.pyramid
                  ? `${ex.pyGrid.length}×${ex.pyGrid[0] ? ex.pyGrid[0].length : 0} · ${ex.totalReps} reps`
