...
window.TransferScreen = function TransferScreen({ exLib, routines, history, circuits, circuitHistory, onImport, onBack }) {
  ...
  const allData = { exLib, routines, history, circuits, circuitHistory, exportedAt: Date.now() };
  ...
  function doImport(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setImportMsg('Import réussi ! ' + (parts.join(' · ') || 'données chargées')); setImportOk(true);
  ...
  useEffectTransfer(() => {
    ...
    const json = JSON.stringify({ exLib, routines, history, circuits, circuitHistory });
    if (json.length > 2000) {
      // Version allégée : on garde les définitions (petites) et on tronque les historiques
      const slim = JSON.stringify({ exLib, routines, circuits, history: history.slice(0, 20), circuitHistory: (circuitHistory||[]).slice(0, 20) });
      ...
      <div style={{ padding:'10px 14px' }}>
        <div style={{ ...S.card, marginBottom:10 }}>
          <div style={{ display:'flex', justifyContent:'space-around', textAlign:'center' }}>
            ...
            <div><div style={{ fontSize:22, fontWeight:700, color:S.orange }}>{(circuitHistory||[]).length}</div><div style={{ fontSize:11, color:'#555', marginTop:2 }}>circuits</div></div>
          </div>
          {(() => {
            const py = history.reduce((a,h) => a + h.exercises.filter(e => e.pyramid).length, 0);
            return py > 0 ? <div style={{ fontSize:11, color:'#555', textAlign:'center', marginTop:10, paddingTop:10, borderTop:'1px solid #1E1E22' }}>dont <span style={{color:S.orange,fontWeight:700}}>{py}</span> exercice{py>1?'s':''} pyramide</div> : null;
          })()}
        </div>
