        ...
        <div key={h.id} style={S.card}>
          ...
          {d.exercises.map((ex, ei) => (
            <div key={ei} style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#aaa", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                {ex.name}
                {ex.pyramid && <span style={{ fontSize: 10, background: "#F59E0B22", color: S.orange, borderRadius: 4, padding: "1px 6px", fontWeight: 700 }}>PYRAMIDE</span>}
              </div>
              {/* Une pyramide ne remplit pas `sets` : on affiche sa grille */}
              {ex.pyramid && ex.pyGrid
                          ...
                          <tr key={r}>
                            ...
                            {row.map((v, c) => (
                              <td key={c} style={{ padding: 0 }}>
                                {editing
                                  ? <input style={{ ...S.inpS, width: 46, textAlign: "center" }} type="number" value={v}
                                      onChange={(e) => setEditD((p) => { const n = JSON.parse(JSON.stringify(p)); n.exercises[ei].pyGrid[r][c] = e.target.value; return n; })}/>
                                  : <div style={{ background: v ? "#1E1E22" : "transparent", border: v ? "1px solid #2A2A2E" : "1px solid #1A1A1E", borderRadius: 6, padding: "5px 4px", textAlign: "center", fontSize: 13, fontWeight: 700, color: v ? "#ccc" : "#444", minWidth: 38 }}>{v || "—"}</div>}
                              </td>
                            ))}
                          </tr>
                        ))}
