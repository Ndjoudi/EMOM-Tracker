...
window.WorkoutScreen=function WorkoutScreen({wo,setWo,onFinish,onCancel,history,exLib,setExLib}){
  ...
  const deadRef=uRW(0),restDeadRef=uRW(0);
  const[runKey,setRunKey]=uSW(0),[restKey,setRestKey]=uSW(0);
  const iRef=uRW(null),tRef=uRW(null),avRef=uRW(null),restRef=uRW(null);
  ...
  uEW(()=>{const tick=()=>setTotalEl(Math.floor((Date.now()-wo.startedAt)/1000));tick();tRef.current=setInterval(tick,1000);return()=>clearInterval(tRef.current);},[]);
  ...
  uEW(()=>{
    ...
    restRef.current=setInterval(()=>{
      ...
      if(left<=0){clearInterval(restRef.current);setRestOn(false);setRestLeft(0);
        window.notify&&window.notify('Repos terminé',`${(wo.exercises[wo.currentExIndex]||{}).name||''} — à toi`);return;}
      setRestLeft(left);
    },250);
  ...
  avRef.current=(endedAt)=>{
    ...
    setWo(prev=>{const n=window.dcw(prev);const ce=n.exercises[n.currentExIndex];const cs=n.currentSet;
      if(ce&&cs<ce.sets.length){ce.sets[cs].done=true;
        if(cs+1<ce.sets.length){n.currentSet=cs+1;
          deadRef.current=base+(ce.emomTime||90)*1000;
          setTimerLeft(Math.max(0,Math.ceil((deadRef.current-Date.now())/1000)));setTimerOn(true);setRunKey(k=>k+1);
          window.notify&&window.notify(`${ce.name} — série ${cs+2}`,"C'est reparti");}
        else{if(n.currentExIndex+1<n.exercises.length){const restDur=ce.restTime||defaultRest;n.currentExIndex+=1;n.currentSet=window.fnu(n.exercises[n.currentExIndex]);
            setDefaultRest(restDur);restDeadRef.current=base+restDur*1000;setRestLeft(restDur);setRestOn(true);setRestKey(k=>k+1);
            window.notify&&window.notify(`${ce.name} terminé`,`Repos ${restDur}s`);}
          else window.notify&&window.notify('Séance terminée',ce.name);
          setTimerOn(false);setTimerLeft(0);}}
      return n;});
  };
  ...
  uEW(()=>{
    ...
    iRef.current=setInterval(()=>{
      ...
      if(left<=0){clearInterval(iRef.current);avRef.current(deadRef.current);return;}
      setTimerLeft(left);
    },250);
  ...
  const start=()=>{window._wantWakeLock=true;window.requestWakeLock&&window.requestWakeLock();window.requestNotifPermission&&window.requestNotifPermission();deadRef.current=Date.now()+emomS*1000;setTimerLeft(emomS);setTimerOn(true);setRunKey(k=>k+1);if(restOn){clearInterval(restRef.current);setRestOn(false);setRestLeft(0);}};
  const stopV=()=>{clearInterval(iRef.current);setTimerOn(false);setTimerLeft(0);avRef.current();};
  const skip=()=>{clearInterval(iRef.current);setTimerOn(false);avRef.current();};
  const adj=d=>{deadRef.current=Math.max(Date.now(),deadRef.current+d*1000);setTimerLeft(Math.ceil((deadRef.current-Date.now())/1000));};
  const adjRest=d=>{restDeadRef.current=Math.max(Date.now(),restDeadRef.current+d*1000);setRestLeft(Math.ceil((restDeadRef.current-Date.now())/1000));};
  ...
  const goEx=i=>{clearInterval(iRef.current);setTimerOn(false);setTimerLeft(0);if(restOn){clearInterval(restRef.current);setRestOn(false);setRestLeft(0);}setWo(p=>({...p,currentExIndex:i,currentSet:window.fnu(p.exercises[i])}));};
  ...
  return(
    <div style={{...S.app,paddingBottom:ex&&ex.pyramid?40:220}}>
      <div style={S.header}>
      ...
      {ex&&(
        <div style={S.card}>
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
            ...
            {ex.pyramid?(
              <button onClick={()=>setShowEditPy({pyRows:String(ex.pyRows||3),pyCols:String(ex.pyCols||5),pyX:String(ex.pyX||60),pyY:String(ex.pyY||120)})} style={{background:'none',border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:6,padding:0,flexShrink:0,fontFamily:'inherit'}}>
                <span style={{display:'flex',alignItems:'center',gap:3,color:S.orange,fontSize:12,fontWeight:600}}><IC.clock/> {Math.floor((ex.pyX||60)/60)}m{(ex.pyX||60)%60>0?((ex.pyX||60)%60)+'s':''} <span style={{color:'#555'}}><IC.edit/></span></span>
                <span style={{background:'#F59E0B22',borderRadius:6,padding:'2px 10px',fontSize:13,fontWeight:700,border:'1px solid #F59E0B44',color:S.orange}}>{ex.pyRows}×{ex.pyCols}</span>
              </button>
            ):(
          ...
          {/* Bloc historique + graphique : masqué en pyramide (alimenté par `sets`,
              que ce mode ne remplit pas — sinon canvas vide qui déborde sur la grille) */}
          {!ex.pyramid&&last2.length>0&&(()=>{
          ...
          {ex&&ex.pyramid?(
            <window.PyramidGrid
              ex={ex}
              onChange={g=>setWo(p=>{const n=window.dcw(p);n.exercises[n.currentExIndex].pyGrid=g;return n;})}
              onDone={v=>setWo(p=>{const n=window.dcw(p);n.exercises[n.currentExIndex].pyDone=v;return n;})}
            />
          ):(()=>{
      ...
      {/* Barre EMOM masquée en pyramide : la grille a son propre timer */}
      <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:480,background:'#131315',borderTop:'1px solid #222',zIndex:30,display:ex&&ex.pyramid?'none':'block'}}>
      ...
      {/* Exo pyramide : cette modale est construite autour des kg et des objectifs de
          charge, sans objet ici. On affiche le tableau des grilles à la place. */}
      {showExStats&&(()=>{
        const isPy=(wo.exercises.find(e=>e.name===showExStats)||{}).pyramid
                 ||history.some(h=>h.exercises.some(e=>e.name===showExStats&&e.pyramid));
        if(!isPy)return null;
        return(
          <Modal onClose={()=>setShowExStats(null)}>
            <ExHistoryTable exName={showExStats} history={history} onClose={()=>setShowExStats(null)}/>
          </Modal>
        );
      })()}
      ...
      {showEditPy&&<Modal onClose={()=>setShowEditPy(null)}>
        <div style={{fontSize:17,fontWeight:700,marginBottom:16}}>Modifier la pyramide</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:8}}>
          <div><label style={S.lbl}>Lignes</label><input style={S.inpS} type="number" value={showEditPy.pyRows} onChange={e=>setShowEditPy(p=>({...p,pyRows:e.target.value}))}/></div>
          <div><label style={S.lbl}>Colonnes</label><input style={S.inpS} type="number" value={showEditPy.pyCols} onChange={e=>setShowEditPy(p=>({...p,pyCols:e.target.value}))}/></div>
          <div><label style={S.lbl}>X — entre séries (sec)</label><input style={S.inpS} type="number" value={showEditPy.pyX} onChange={e=>setShowEditPy(p=>({...p,pyX:e.target.value}))}/></div>
          <div><label style={S.lbl}>Y — repos (sec)</label><input style={S.inpS} type="number" value={showEditPy.pyY} onChange={e=>setShowEditPy(p=>({...p,pyY:e.target.value}))}/></div>
        </div>
        <div style={{fontSize:12,color:'#666',marginBottom:16}}>Les reps déjà saisies sont conservées.</div>
        <button style={S.btn} onClick={()=>{
          ...
          setWo(p=>{
            const n=window.dcw(p);const e=n.exercises[n.currentExIndex];const old=e.pyGrid||[];
            // Redimensionne en gardant les cases communes
            e.pyGrid=Array.from({length:rows},(_,r)=>Array.from({length:cols},(_,c)=>(old[r]&&old[r][c]!=null)?old[r][c]:''));
            e.pyRows=rows;e.pyCols=cols;e.pyX=X;e.pyY=Y;
            return n;
          });
          setShowEditPy(null);
        }}>Enregistrer</button>
