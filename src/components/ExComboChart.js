...
window.ExComboChart = function ExComboChart({
  exName,
  hist,
  goal,
  height,
  ...
  uEWC(() => {
    ...
    // ── raw data historique ──
    const rawData = [...hist].reverse().flatMap(entry => entry.exercises.filter(e => e.name === exName).map(e => ({
      date: entry.date,
    ...
    if (targetCurve.length > 0 && !compact) {
      ...
      grouped.forEach(d => allMap.set(d.label, {
        vol: d.vol,
        kg: d.maxKg,
        tgt: null,
        date: d.date
      ...
      targetCurve.forEach(t => {
        if (!allMap.has(t.label)) allMap.set(t.label, {
          vol: null,
          kg: null,
          tgt: t.kg,
          date: t.date
    ...
    const maxY = compact ? kgVals.length ? Math.max(...kgVals) + margin : undefined : goal ? Math.max(goal.kg + 5, ...kgVals) : undefined;
    ...
    if (targetKg && targetKg > 0) {
      ...
      tgtData.push(null);
      datasets.forEach(d => {
        if (Array.isArray(d.data)) d.data.push(null);
      });
      datasets.push({
        type: 'line',
        data: prevData,
      ...
      options: {
        ...
        scales: {
          ...
          yVol: {
            ...
            ...(compact && volData.filter(Boolean).length ? {
              min: Math.min(...volData.filter(Boolean)) * 0.95,
              max: Math.max(...volData.filter(Boolean)) * 1.05
            } : {}),
            ticks: {
              color: '#22C55E66',
              font: {
  ...
  return /*#__PURE__*/React.createElement("div", null, !compact && /*#__PURE__*/React.createElement("div", {
    ...
    style: btnStyle('seances'),
    onClick: () => setPeriod('seances')
  }, "S\xE9ances"), /*#__PURE__*/React.createElement("button", {
    style: btnStyle('semaines'),
    onClick: () => setPeriod('semaines')
  }, "Semaines"), /*#__PURE__*/React.createElement("button", {
    style: btnStyle('mois'),
